...
| DeepSeek Flash: 17% de respuestas vacías en banco de 4 tareas × 3 reps (GLM 5.3 Flash: 0%); las vacías se cobran igual | Proveedor/modelo inestable | Fallback con `models` + `route: 'fallback'` |
...
| `models` + `route: 'fallback'` | Respaldo automático entre modelos en la misma petición |
...
## El mismo cuerpo para dos modelos

Con `models` + `route:'fallback'`, OpenRouter manda el MISMO cuerpo al modelo
al que enrute. No hay un parámetro de razonamiento óptimo para ambos:

| | `{enabled:false}` | `{effort:"low"}` |
|---|---|---|
| `deepseek/deepseek-v4.1-flash` | $0.0000050 | $0.0000124 |
| `z-ai/glm-5.3-flash` | **400 Reasoning is mandatory** | $0.0000071 |

Usamos `effort:"low"`: cuesta $0.0000074 más por llamada en DeepSeek y evita
que el respaldo falle entero.
